<?php
    ...
    $txtTipoPersonaId="7";    // contacto
    $txtStatusId="9";
    $txtPersonaActId="0";
    $inputAgenteId="0";
    $txtAgenteId="register.php";
    $inputPlanId="0";   
    $inputPorcentaje=""; 
    $txtPerfilId="14";  // contacto
    $txtOtrosPlanes="0";
    ...
    if ($_GET){
        ...
        {  $inputAgenteId=$_GET["agente_id"];                      
           if (is_numeric($_GET["agente_id"])){
                  $txtAgenteId="register.php"."?agente_id=".$inputAgenteId; 
                  $inputReferidoPor=referidoPor($inputAgenteId,$conexion);
           } else {$txtAgenteId="register.php";
                   $inputAgenteId="0";
                   $inputReferidoPor="";
                   }
...
if ($_POST){
    ...
    $inputPlanId=( isset($_POST["btnradio2"]) )? $_POST["btnradio2"]:""  ;             

    if ($inputPlanId==9)
       {
             $txtOtrosPlanes=9;  // real team
             $inputPlanId=7; // embajador
       }
    
   if ($inputPlanId==10)
       {
             $txtOtrosPlanes=10;  // mentoria
             $inputPlanId=0;
       }
    ...
    $txtIntereses9=  (isset($_POST["cbIntereses9"]))?$_POST["cbIntereses9"]:"";        
    ...
    $txtIntereses= ($txtIntereses!="") 
                    ? $txtIntereses. ((isset($txtIntereses9) && ($txtIntereses9!="")) ?"-".$txtIntereses9:"")
                    : ((isset($txtIntereses9)) ?$txtIntereses9:"");                    
    ...
    try {
        ...
        if($cuentaYaExiste=="0"){
                    ...
                    if (isset($resultado["id"])){
                        ...
                        //  aqui va  buscar el porcentaje
                        $sql="SELECT * FROM tb_planes_comisiones WHERE id=:plan_id";
                        $sentenciaSQL=$conexion->prepare($sql);
                        $sentenciaSQL->bindParam(":plan_id",$inputPlanId);           
                        $sentenciaSQL->execute();   
                        
                        $resultado=$sentenciaSQL->fetch(PDO::FETCH_LAZY); 
                        if (isset($resultado["id"]))
                        {
                          $inputPorcentaje=$resultado["porc_agente"];
                          $txtPerfilId=$resultado["perfil_id"];                          
                        }  else    $inputPorcentaje="0";


                        // aqui va a buscar a quien lo refirio si escribió un numero de ID
                        if ($inputAgenteId=="0" ||  $inputAgenteId=="")
                        {
                                $sql="SELECT * FROM tb_personas_datos_extra WHERE personas_id=:personas_id";
                                $sentenciaSQL=$conexion->prepare($sql);
                                $sentenciaSQL->bindParam(":personas_id",$inputReferidoPor);           
                                $sentenciaSQL->execute();   
                                
                                $resultado=$sentenciaSQL->fetch(PDO::FETCH_LAZY); 
                                if (isset($resultado["personas_id"]))  // si lo encuentra por ID toma sus datos
                                {
                                $inputReferidoPor=$resultado["nombre_completo"];
                                $inputAgenteId=$resultado["personas_id"];
                                }
                        }  
                        ...
                        agente_id, referido_por_id, status_id , idioma_id, perfil_id, oasis_id, company_name) 
                        values (:personas_id, :nombre, :apellido, :fecha_nacimiento,
                        :intereses, :telefono1, :nombre_completo, :tipo_persona_id,
                        :fecha_cap, :fecha_act, :idioma, :persona_act_id, :correo, :user_login, :referido_por, 
                        :agente_id, :referido_por_id, :status_id, (select  id from tb_idiomas where descripcion=:idioma2), :perfil_id, 1 ,'Oasis Inmobiliario')" ;
                        $sentenciaSQL=$conexion->prepare($sql);
                        ...
                        $sentenciaSQL->bindParam(":perfil_id",$txtPerfilId);                                                          
                        $sentenciaSQL->bindParam(":agente_id",$inputAgenteId);           
                        $sentenciaSQL->bindParam(":referido_por_id",$inputAgenteId);                                   
                        ...
                        // Insertar datos de los embajadores y colaboradores                        
                        if( ($inputPlanId==7) or ($inputPlanId==8) )
                        {                     
                            $sql="INSERT INTO tb_agentes_datos (persona_id,  referal_for, plan_comision_id, porc_comision) 
                            values (:bdPersonasId,  :referal_for, :plan_comision_id,    :porc_comision) " ;                            
                            $sentenciaSQL=$conexion->prepare($sql);                        
                            $sentenciaSQL->bindParam(":referal_for",$inputReferidoPor);   
                            $sentenciaSQL->bindParam(":porc_comision",$inputPorcentaje);                           
                            $sentenciaSQL->bindParam(":plan_comision_id",$inputPlanId);                                                   
                            $sentenciaSQL->bindParam(":bdPersonasId",$bdPersonasId);                                   
                            $sentenciaSQL->execute();     
                        }
                        ...
                        if($inputPlanId==7)
                        {
                            if($txtOtrosPlanes==9)
                            {
                                // Real team aqui abajo poner el link de pago de realteam
                                header("Location: https://oasisinmobiliario.com/the-real-team");                                                                 
                                die();                                
                            }
                            else
                            {
                                header("Location: https://oasisinmobiliario.com/embajador");                                                                 
                                die();
                            }
                        }
                        else
                        {  if ($inputPlanId==8)
                              {
                                header("Location:https://oasisinmobiliario.com/colaborador");                                     
                                die();                                
                              }  
                            else
                            { 
                                if ($txtOtrosPlanes==10)
                                {
                                    // Mentoria aqui abajo poner el link de mentoria
                                    header("Location:https://oasisinmobiliario.com/mentoria");                                     
                                    die();                                                                    
                                }
                            }
                        }
...
<html lang="en">
    <head>
        ...
        <title>Register - Oasis Inmobiliario</title>
                                        ...
                                        <form action=<?php echo $txtAgenteId ;?>     method="post">
                                            ...
                                            <div class="row mb-3">                                                
                                                <div class="col-md-6">
                                                    ...
                                                    <div class="form-check">    
                                                    <input <?php echo ($txtIntereses9!="")?"checked":""; ?> name="cbIntereses9" value="Oasis Inmobiliario" class="form-check-input" type="checkbox"  id="">
                                                    <label class="form-check-label" for="">
                                                        Oasis inmobiliario
                                                    </label>
                                                    </div>                                                       
                                                ...
                                                <div class="col-md-6">                                                                                                
                                                    <div class="form-floating mb-3 mb-md-0">                                                  
                                                                ...
                                                                <div  class="btn-group" role="group" aria-label="Basic checkbox toggle button group">                                                            
                                                                    ...
                                                                    <input value="0" type="radio" class="btn-check" checked name="btnradio2" id="btncheck4" autocomplete="off">
                                                                    <label class="btn btn-outline-primary" for="btncheck4">No aplica</label>

                                                                    <input value="7" type="radio" class="btn-check"  name="btnradio2" id="btncheck5" autocomplete="off">
                                                                    <label class="btn btn-outline-primary" for="btncheck5">Embajador</label>

                                                                    <input value="8" type="radio" class="btn-check" name="btnradio2" id="btncheck6" autocomplete="off">
                                                                    <label class="btn btn-outline-primary" for="btncheck6">Colaborador</label>

                                                                    <input value="9" type="radio" class="btn-check" name="btnradio2" id="btncheck7" autocomplete="off">
                                                                    <label class="btn btn-outline-primary" for="btncheck7">The RealTeam</label> 

                                                                    <input value="10" type="radio" class="btn-check" name="btnradio2" id="btncheck8" autocomplete="off">
                                                                    <label class="btn btn-outline-primary" for="btncheck8">Mentoria</label>                                                                                                                                        
